...
const EXPECTED_REPOSITORY_URL = "https://github.com/simolt/simolt";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "simolt") {
    errors.push(`package.json name must be "simolt"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.simolt !== "simolt.mjs") {
    errors.push(`package.json bin.simolt must be "simolt.mjs"; found "${pkg.bin?.simolt ?? ""}".`);
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`simolt-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `simolt-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
